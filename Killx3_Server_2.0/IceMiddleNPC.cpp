...
CIceMiddleNPC::CIceMiddleNPC(WORD ID, Vec3 _InitPos, float _Speed, BYTE _MoveState,
...
CIceMiddleNPC::~CIceMiddleNPC() {}

void CIceMiddleNPC::SetID(int _ID) {
	m_ID = _ID;
}
Vec3 CIceMiddleNPC::GetPos() { return m_Position; }
bool CIceMiddleNPC::GetAlive() { return m_alive; }
Vec3 CIceMiddleNPC::GetInitPos() { return m_InitPos; };

void CIceMiddleNPC::Update(float DeltaTime, HANDLE _hIOCP, CLIENT * p_Clients, const int& ID1, const int& ID2)
{
	if (m_HP <= 0)
		m_alive = false;
...
void CIceMiddleNPC::SetDestPoint(Vec3 _DestVec3) {
	DestPoint.push(_DestVec3);
}
void CIceMiddleNPC::SetAniState(BYTE _Ani)
{
	m_AnimationState = _Ani;
}
void CIceMiddleNPC::SetHP(int _Hp)
{
	m_HP -= _Hp;
}
void CIceMiddleNPC::SetBackDist(float dist)
{
	m_BackDist = dist;
}
void CIceMiddleNPC::SetRemove(bool remove)
{
	m_Remove = remove;
}
void CIceMiddleNPC::SetMapType(BYTE map)
{
	m_MapType = map;
}
queue<Vec3> CIceMiddleNPC::GetDestPoint() {
	return DestPoint;
}
void CIceMiddleNPC::SetDir(Vec3 _Dir) {
	m_Direction = _Dir;
}
void CIceMiddleNPC::SetSpeed(float _Speed)
{
	MonsterSpeed = _Speed;
}
void CIceMiddleNPC::SetRoomNumber(int _RoomNumber) {
	RoomNumber = _RoomNumber;
}
int CIceMiddleNPC::GetRoomNumber() {
	return RoomNumber;
}
BYTE CIceMiddleNPC::GetAniState()
{
	return m_AnimationState;
}
int CIceMiddleNPC::GetHP()
{
	return m_HP;
}
float CIceMiddleNPC::GetBackDist()
{
	return m_BackDist;
}
bool CIceMiddleNPC::GetRemove()
{
	return m_Remove;
}
Vec3 CIceMiddleNPC::GetDir() {
	return m_Direction;
}

float CIceMiddleNPC::GetSpeed()
{
	return MonsterSpeed;
}
