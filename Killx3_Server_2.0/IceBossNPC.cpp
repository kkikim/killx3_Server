...
CIceBossNPC::CIceBossNPC(WORD ID, Vec3 _InitPos, float _Speed, BYTE _MoveState,
...
CIceBossNPC::~CIceBossNPC() {}

void CIceBossNPC::SetID(int _ID) {
	m_ID = _ID;
}
Vec3 CIceBossNPC::GetPos() { return m_Position; }
bool CIceBossNPC::GetAlive() { return m_alive; }
Vec3 CIceBossNPC::GetInitPos() { return m_InitPos; };

void CIceBossNPC::Update(float DeltaTime, HANDLE _hIOCP, CLIENT * p_Clients, const int& ID1, const int& ID2)
{
	if (m_Lobby)
		return;
	...
	if (!m_alive)
	{
		...
		if (m_RemoveTime > 10.0f)
		{
			...
			send_over->operation = OP_ICELOBBY_BACK;
			printf("A1\n");
...
void CIceBossNPC::SetDestPoint(Vec3 _DestVec3) {
	DestPoint.push(_DestVec3);
}
void CIceBossNPC::SetAniState(BYTE _Ani)
{
	m_AnimationState = _Ani;
}
void CIceBossNPC::SetHP(int _Hp)
{
	m_HP -= _Hp;
}
void CIceBossNPC::SetBackDist(float dist)
{
	m_BackDist = dist;
}
void CIceBossNPC::SetRemove(bool remove)
{
	m_Remove = remove;
}
void CIceBossNPC::SetMapType(BYTE map)
{
	m_MapType = map;
}
queue<Vec3> CIceBossNPC::GetDestPoint() {
	return DestPoint;
}
void CIceBossNPC::SetDir(Vec3 _Dir) {
	m_Direction = _Dir;
}
void CIceBossNPC::SetSpeed(float _Speed)
{
	MonsterSpeed = _Speed;
}
void CIceBossNPC::SetRoomNumber(int _RoomNumber) {
	RoomNumber = _RoomNumber;
}
int CIceBossNPC::GetRoomNumber() {
	return RoomNumber;
}
BYTE CIceBossNPC::GetAniState()
{
	return m_AnimationState;
}
int CIceBossNPC::GetHP()
{
	return m_HP;
}
float CIceBossNPC::GetBackDist()
{
	return m_BackDist;
}
bool CIceBossNPC::GetRemove()
{
	return m_Remove;
}
Vec3 CIceBossNPC::GetDir() {
	return m_Direction;
}

float CIceBossNPC::GetSpeed()
{
	return MonsterSpeed;
}
