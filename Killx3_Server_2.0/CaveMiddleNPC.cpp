...
CCaveMiddleNPC::CCaveMiddleNPC(WORD ID, Vec3 _InitPos, float _Speed, BYTE _MoveState, float _Dist, int _HP, float _AttackDist,BYTE _mapType)
{//생성자 인자로 방향도 초기화해저야되
	m_ID = ID;
	m_Target = -1;
	m_alive = true;
...
CCaveMiddleNPC::~CCaveMiddleNPC() {}

void CCaveMiddleNPC::SetID(int _ID) {
	m_ID = _ID;
}
Vec3 CCaveMiddleNPC::GetPos() { return m_Position; }
bool CCaveMiddleNPC::GetAlive() { return m_alive; }
Vec3 CCaveMiddleNPC::GetInitPos() { return m_InitPos; };

void CCaveMiddleNPC::Update(float DeltaTime, HANDLE _hIOCP, CLIENT * p_Clients, const int& ID1, const int& ID2)
{
	if (m_HP <= 0)
		m_alive = false;
...
void CCaveMiddleNPC::SetDestPoint(Vec3 _DestVec3) {
	DestPoint.push(_DestVec3);
}
void CCaveMiddleNPC::SetAniState(BYTE _Ani)
{
	m_AnimationState = _Ani;
}
void CCaveMiddleNPC::SetHP(int _Hp)
{
	m_HP -= _Hp;
}
void CCaveMiddleNPC::SetBackDist(float dist)
{
	m_BackDist = dist;
}
void CCaveMiddleNPC::SetRemove(bool remove)
{
	m_Remove = remove;
}
queue<Vec3> CCaveMiddleNPC::GetDestPoint() {
	return DestPoint;
}
void CCaveMiddleNPC::SetDir(Vec3 _Dir) {
	m_Direction = _Dir;
}
void CCaveMiddleNPC::SetSpeed(float _Speed)
{
	MonsterSpeed = _Speed;
}
void CCaveMiddleNPC::SetRoomNumber(int _RoomNumber) {
	RoomNumber = _RoomNumber;
}
int CCaveMiddleNPC::GetRoomNumber() {
	return RoomNumber;
}
BYTE CCaveMiddleNPC::GetAniState()
{
	return m_AnimationState;
}
int CCaveMiddleNPC::GetHP()
{
	return m_HP;
}
float CCaveMiddleNPC::GetBackDist()
{
	return m_BackDist;
}
bool CCaveMiddleNPC::GetRemove()
{
	return m_Remove;
}
Vec3 CCaveMiddleNPC::GetDir() {
	return m_Direction;
}

float CCaveMiddleNPC::GetSpeed()
{
	return MonsterSpeed;
}
